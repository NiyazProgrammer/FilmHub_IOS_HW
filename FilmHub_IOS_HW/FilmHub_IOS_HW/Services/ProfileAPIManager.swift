...
protocol ProfileAPIManagerNetworkServices {
    func loadUsersFilms(with recentFilmId: [Int], completion: @escaping ([Movie]?) -> Void)
}
// MARK: For profile screen
class ProfileAPIManager: ProfileAPIManagerNetworkServices {
    ...
    func loadUsersFilms(with filmsId: [Int], completion: @escaping ([Movie]?) -> Void)  {
        var movies: [Movie] = []
